// Os metodos avançados de array são formas mais "modernas" de iterar sobre arrays

// MAP

// O MAP NÃO ALTERA O SEU ARRAY ORIGINAL, ELE TE RETORNA UM NOVO ARRAY "MODIFICADO"
// A GENTE PODE (E AS VEZES DEVE) GUARDAR O RESULTADO DO MAP EM UMA NOVA VARIAVEL

// O QUE O MAP FAZ? ELE INVOCA UM CALLBACK PARA CADA ELEMENTO DE UM ARRAY E VAI GUARDAR O RETORNO DESSA CALLBACK NA NOVA ARRAY QUE RETORNA
...
// Criar uma array de strings que guarda o nome dos filmes

// function callbackDoMeuMap(currentMovie) {
//   return currentMovie.original_title;
// }

// const callbackDoMeuMap = (currentMovie) => currentMovie.original_title;

// const soNomesDosFilmes = movies.map(callbackDoMeuMap);

// console.log(soNomesDosFilmes);

// FAZENDO A MESMA COISA COM CALLBACK ANONIMA

const onlyTitles = movies.map(function (currentMovie) {
  return currentMovie.original_title;
});

// console.log(onlyTitles);

// Fazendo a mesma coisa com ARROW FUNCTION ANONIMA

const titulosApenas = movies.map((currentMovie) => currentMovie.original_title);

const algo = movies.forEach((currentMovie) => {
  console.log(`O filme é ${currentMovie.title}`);
});

// OBS: A ARRAY QUE O MAP TE DER SEMPRE, SEMPRE, SEMPREEEEEEEEEEE VAI TER O MESMO TAMANHO DO ARRAY ORIGINAL. MESMO QUE ELE PRECISE PREENCHER BURRACOS NA CARECA DO MAX COM UNDDEFINED

// FILTER

// Serve pra filtrar BADUM TISSSSSS

// Ele NÃO muda o array original, igual o map
// Ele te retorna um novo array com os elementos filtrados
// Os elementos que estiverem dentro do novo array sempre vão ser EXATAMENTE IGUAIS aos do array original
// Como ele filtra? Basicamente a callback q ele recebe precisa retornar um booleano. Se esse booleano for true, aquele currentElement entra, se for false, ele ta fora

// const filmesFiltradosPelaNota = movies.filter((currentMovie) => {
//   return currentMovie.vote_average > 7.8;
// });

const filmesFiltrados = movies.filter(
  (currentMovie) => currentMovie.vote_average > 7.8
);

console.log(filmesFiltradosPelaNota);
